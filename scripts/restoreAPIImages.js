...
// Original API image URLs from Autotrader
const apiImages = [
...
async function restoreAPIImages() {
  try {
    console.log('Restoring original API images...');
    ...
    for (let i = 0; i < vehicles.length; i++) {
      ...
      const originalImagePath = apiImages[imageIndex];
      
      await VehicleModel.findByIdAndUpdate(vehicle._id, {
        imagePath: originalImagePath
      });
      
      console.log(`Restored ${vehicle.title} with original API image ${imageIndex + 1}`);
    }
    
    console.log('All vehicle images restored to original API images!');
  } catch (error) {
    console.error('Error restoring API images:', error);
  } finally {
    mongoose.disconnect();
  }
}
